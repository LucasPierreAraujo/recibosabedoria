...
export default function MembrosPage() {
  ...
  const [formData, setFormData] = useState({ nome: '', grau: '', status: 'ATIVO' });
  ...
  const carregarMembros = async () => {
    try {
      const response = await fetch('/api/membros', { credentials: 'include' });
  ...
  const handleEdit = (membro) => {
    setFormData({ nome: membro.nome, grau: membro.grau, status: membro.status });
      ...
      <main className="max-w-7xl mx-auto p-8">
        {showForm && (
          <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
            <h2 className="text-xl font-bold mb-4 text-gray-800">{editingId ? 'Editar Membro' : 'Novo Membro'}</h2>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow-lg p-8 text-center text-gray-600">Nenhum membro cadastrado</div>
        ) : (
                ...
                {membros.map((membro) => (
                  <tr key={membro.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button onClick={() => handleEdit(membro)} className="text-blue-600 hover:text-blue-800 mr-4">
                        <Edit size={20} />
                      </button>
