...
export default function EditarAtaPage() {
  ...
  // Presenças
  const [presencasQuadroIds, setPresencasQuadroIds] = useState([]); // Array of IDs
  ...
  const carregarAta = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setNumeroAta(ata.numeroAta);
        setLivro(ata.livro);
        setData(new Date(ata.data).toISOString().split('T')[0]);
        setHorarioInicio(ata.horarioInicio);
        setHorarioEncerramento(ata.horarioEncerramento);
        setNumeroPresentes(ata.numeroPresentes);
        setValorTronco(Number(ata.valorTronco).toFixed(2).replace('.', ','));

        // Preencher conteúdo
        setLeituraAta(ata.leituraAta || '');
        setExpediente(ata.expediente || '');
        setOrdemDia(ata.ordemDia || '');
        ...
        const presencasQuadroIds = ata.presencas
          .filter(p => p.tipo === 'QUADRO' && p.membroId)
          .map(p => p.membroId);
        setPresencasQuadroIds(presencasQuadroIds);

        const presencasVisitantesFormatadas = ata.presencas
          .filter(p => p.tipo === 'VISITANTE')
          .map(p => p.nomeManual || '');
        setPresencasVisitantes(presencasVisitantesFormatadas);
  ...
  const salvarAta = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Erro ao atualizar ata: ${error.error}`);
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <header className="bg-blue-900 text-white p-4 shadow-lg">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold">Editar Ata</h1>
              <p className="text-sm text-blue-200">Alterar dados da ata {numeroAta}</p>
            </div>
          </div>
          <button
            onClick={salvarAta}
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="flex justify-end">
          ...
          >
            <Save size={24} /> {salvando ? 'Salvando...' : 'Salvar Alterações'}
          </button>
        </div>
      </main>
    </div>
